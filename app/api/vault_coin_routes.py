...
@login_required
def get_coins(vault_id):
    vault_coins = VaultCoin.query.filter_by(vault_id=vault_id).all()
...
@login_required
def get_one_coin(vault_id, coin_id):
    vault_coin = VaultCoin.query.filter_by(vault_id=vault_id).filter_by(coin_id=coin_id)
